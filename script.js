...
function formGrid(){
  ...
  if(isGrid){
      ...
      height = 0;
      while (height <= south){
        var randNum = Math.ceil(  Arbitrary(1,10));
...
function leaders(){
  ...
  for (var i = 0; i < leaderCount; i++){
      ...
      var marker = new google.maps.Marker({
            icon: BitmapDescriptorFactory.fromBitmap('http://icons.iconarchive.com/icons/icons8/ios7/128/Industry-Radio-Tower-icon.png'),
   ...
   var botL = new google.maps.Marker({
    ...
    var botR = new google.maps.Marker({
     ...
     /* clearMarkerPosition(infrastructureMarkers, infrastructurePositions);
      ...
      var ne = bounds.getNorthEast();
      alert("tester",sw);
      var point = new google.maps.LatLng(sw);
      ...
      for(var i = 1; i < noOfUsers/5; i++){
          var point = new google.maps.LatLng(bounds.getNorthEast().lat(), bounds.getSouthWest().lat());
          infrastructurePositions.push(point);
